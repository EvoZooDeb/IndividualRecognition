...
q3_x = 610                            # Decrease x size
q3_y = 410                            # Decrease y size
alpha = 0.2                           # Triplet loss variable
lr = 0.005                            # learning rate at start
epochs = 100                          
columns_shape = 6                     # Shape of the reading csv file
dimensions = 8                        # Output dimensions
knn_neighbors = 8                     # Common neighbor number
output_model_file_name = "/home/dkatona/tanitas_ind32.h5"
validation_x_epoch = 2
change_lr_x_epoch = 15
prediction_info_at_x_epoch = 2
...
def prediction_calculation(output_message,fitting_images,fitting_classes,training_images,training_classes,fit_or_not = False):
    ...
    if training_images != "0" and training_classes != "0":
        train_prediction = model1.predict(training_images, batch_size = 1)
        nbrs = NearestNeighbors(n_neighbors=knn_neighbors).fit(train_prediction)
        distances, indices = nbrs.kneighbors(prediction)
        top5_class_idx = train_classes[indices]
        
        counter = 0
        for i in range(len(fitting_classes)):
            if sum(np.isin(top5_class_idx[i],fitting_classes[i])) > 0:
                counter+=1
        
        top5_true_acc = counter/len(fitting_classes)
        print("TOP5 accuracy: ", str(top5_true_acc))
    else:
        # Simple printing
...
#outputs = Lambda(lambda t: tf.linalg.normalize(t, ord = np.inf, axis=1)[0])(outputs)
...
horse_mixed = ["/home/wildhorse_project/detectron_pic/datasets/training_dataset_32ind_.csv"]
horse_val_mixed = ["/home/wildhorse_project/detectron_pic/datasets/validation_dataset_32ind_.csv"]
...
for i in range(epochs):
    if i%change_lr_x_epoch == (change_lr_x_epoch-1):
        optimizer.learning_rate=lr*0.5
    if i%prediction_info_at_x_epoch == (prediction_info_at_x_epoch-1): 
        t_acc = prediction_calculation("Training accuracy: ",train_images,train_classes,"0","0",True)
        v_acc = prediction_calculation("Validation accuracy: ",validation_images,validation_classes,"0","0")
        top5_acc = prediction_calculation("TOP 5: Validation accuracy: ",validation_images,validation_classes,train_images,train_classes)
