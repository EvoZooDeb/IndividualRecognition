...
# Validation pictures
szazxszep_val="/home/wildhorse_project/detectron_pic/FINAL_TEST/szazxszep_val.csv"
thetisz_val="/home/wildhorse_project/detectron_pic/FINAL_TEST/thetisz_val.csv"
...
szazxszep_6th="/home/wildhorse_project/detectron_pic/FINAL_TEST/szazxszep_6th.csv"
thetisz_6th="/home/wildhorse_project/detectron_pic/FINAL_TEST/thetisz_6th.csv"
...
val_horses = [szazxszep_val,thetisz_val,panka_val,huba_val,lilla_val,lantos_val]                                                                       # Video validation
...
less_horses = [szazxszep_6th,thetisz_6th,panka_6th,huba_6th,lilla_6th,lantos_6th]                                                                      # Every 6th images
...
for i in range(epochs):
    ...
    if i%2 == 1:    
        ...
        conf_matrix = confusion_matrix(val_res, val_classes)+(np.ones(shape = (len(all_classes),len(all_classes)))*10)
        np.fill_diagonal(conf_matrix, 0)
        print(val_res)
    ...
    for j in np.random.permutation( range(len(my_data)) ):
        ...
        if p == j:
            p = np.random.choice(np.where( (my_data[:,1] == my_data[j,1]))[0],1)[0]
        p_class = int(my_data[j,1])-1
        
        n_column = random.choices(population=all_classes, weights=conf_matrix[:,p_class], k=1)
        n = np.random.choice(np.where(my_data[:,1] == str(n_column[0]))[0],1)[0]                           # Get random id from another class
    ...
    model1.save("/home/dkatona/regulizer_1e1_weights.h5")
