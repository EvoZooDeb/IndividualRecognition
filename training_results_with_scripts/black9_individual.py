...
horse_black=[gerle_black,huba_black,lantos_black,lilla_black,noci_black,panka_black,szazxszep_black,thetisz_black,vadoc_black]
horse_black_val=[gerle_black_val,huba_black_val,lantos_black_val,lilla_black_val,noci_black_val,panka_black_val,szazxszep_black_val,thetisz_black_val,vadoc_black_val]
...
# Black 200 horse
my_data2 = np.empty(shape=(0,3))
for i in horse_black:
    my_data2 = np.concatenate((my_data2,np.genfromtxt(i, dtype=str, delimiter=',')),axis = 0)

# Black val 200 horse
val_data_black = np.empty(shape=(0,3))
for i in horse_black_val:
    val_data_black = np.concatenate((val_data_black,np.genfromtxt(i, dtype=str, delimiter=',')),axis = 0)
...
val_black_images = read_image(val_data_black[:,0])
val_black_classes = np.array(val_data_black[:,1].astype(int))
...
for i in range(epochs):
    ...
    if i%2 == 1:    
        ...
        val_black_pred = model1.predict(val_black_images, batch_size = 1)
        val_black_pred = tf.linalg.normalize(val_black_pred, axis = 1)[0]
...
#         val_plus_pred_crop_black = tf.linalg.normalize(val_plus_pred_crop_black, axis = 1)[0] 
        ...
        val_black_res = clf.predict(val_black_pred)
...
#         val_plus_res_crop_black = clf.predict(val_plus_pred_crop_black)
        ...
        val_black_accuracy = sum(val_black_res == val_black_classes)/val_data_black.shape[0]
...
#         plus_accuracy_crop_black = sum(val_plus_res_crop_black == val_plus_classes_crop_black)/val_plus_data_crop_black.shape[0]
        ...
        conf_matrix = confusion_matrix(val_res, val_classes)+(np.ones(shape = (len(all_classes),len(all_classes))))
        np.fill_diagonal(conf_matrix, 0)
#         print(val_res)
        ...
        print(val_black_accuracy)
        print("CONF_MATRIX val_black_accuracy")
        print(confusion_matrix(val_black_res, val_black_classes))
#         print(val_plus_res)
